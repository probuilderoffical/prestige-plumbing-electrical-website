...
const counter=setInterval(()=>{n=Math.min(100,n+Math.ceil(Math.random()*18));count.textContent=String(n).padStart(2,'0');if(n===100){clearInterval(counter);setTimeout(()=>{boot.classList.add('done');document.body.classList.remove('lock')},80)}},35);
...
function sizeCanvas(){const d=Math.min(devicePixelRatio,1.5);canvas.width=innerWidth*d;canvas.height=innerHeight*d;canvas.style.width=innerWidth+'px';canvas.style.height=innerHeight+'px';ctx.setTransform(d,0,0,d,0,0);const total=Math.min(52,Math.max(28,Math.floor(innerWidth/25)));particles=Array.from({length:total},(_,i)=>({x:Math.random()*innerWidth,y:Math.random()*innerHeight,vx:(Math.random()-.5)*.2,vy:(Math.random()-.5)*.2,r:Math.random()*1.2+.3,c:i%4===0?'77,167,255':'98,215,239'}))}
let previousFrame=0;function draw(time=0){requestAnimationFrame(draw);if(time-previousFrame<32)return;previousFrame=time;ctx.clearRect(0,0,innerWidth,innerHeight);for(const p of particles){p.x+=p.vx+(mx-.5)*.035;p.y+=p.vy+(my-.5)*.035;if(p.x<0)p.x=innerWidth;if(p.x>innerWidth)p.x=0;if(p.y<0)p.y=innerHeight;if(p.y>innerHeight)p.y=0;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle=`rgba(${p.c},.34)`;ctx.fill()}for(let i=0;i<particles.length;i++){for(let j=i+1;j<particles.length;j++){const a=particles[i],b=particles[j],dx=a.x-b.x,dy=a.y-b.y,d=Math.hypot(dx,dy);if(d<100){ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.strokeStyle=`rgba(70,130,160,${(1-d/100)*.07})`;ctx.stroke()}}}}
...
// Tactile click feedback on every interactive control.
$$('a,button').forEach(el=>el.addEventListener('pointerdown',event=>{const r=el.getBoundingClientRect(),wave=document.createElement('span');wave.className='click-wave';const size=Math.max(r.width,r.height)*1.8;wave.style.cssText=`width:${size}px;height:${size}px;left:${event.clientX-r.left-size/2}px;top:${event.clientY-r.top-size/2}px`;el.classList.add('pressed');el.appendChild(wave);setTimeout(()=>el.classList.remove('pressed'),180);setTimeout(()=>wave.remove(),650)}));
